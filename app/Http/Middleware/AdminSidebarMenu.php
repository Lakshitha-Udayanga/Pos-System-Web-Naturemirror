            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if($super_admin->enable_sale_bulk_return != 0){
                            if (auth()->user()->can('access_sell_return') || auth()->user()->can('access_own_sell_return')) {
                                $sub->url(
                                    action('SaleBulkReturnController@index'),
                                    __('bulk_sell_return.bulk_sell_return'),
                                    ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'bulk-sell-return']
                                );
                            }
                        }
                    ...
                    function ($sub) {
                        ...
                        if($super_admin->advance_stock_adjustment != 0){
                            ...
                            if (auth()->user()->can('purchase.create')) {
                                ...
                                $sub->url(
                                    action('StockAdjustmentController@createForAdd'),
                                    __('Increase Stock'),
                                    ['icon' => 'fa fas fa-circle', 'active' => request()->segment(2) == 'create-for-add']
                                );
                            }
            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        // if($super_admin->cheque_module == 1){
                            
                            $sub->url(
                                action('ChequesController@index'),
                                __('Cheque Management'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'cheque']
                            );
                        // }
                        $sub->url(
