        ...
        if ($request->ajax()) {
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw('SUM(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) as total_qty_sold'),
                    DB::raw('SUM(transaction_sell_lines.free_issue_quantity) as total_free_issue_quantity'),
                    DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as subtotal'),
                    'transaction_sell_lines.parent_sell_line_id',
                DB::raw("SUM(IF(t.type='sell', t.free_issues_amount, 0)) as free_issues_amount"),
                );
            ...
            return Datatables::of($query)
                ...
                ->editColumn('total_free_issue_quantity', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency free_qty" data-currency_symbol=false data-orig-value="' . (float)$row->total_free_issue_quantity . '" data-unit="" >' . (float) $row->total_free_issue_quantity . '</span> ' . $row->unit;
                })
                ->editColumn('current_stock', function ($row) {
                ...
                ->editColumn('free_issues_amount', function ($row) {
                    $class = is_null($row->parent_sell_line_id) ? 'free_issues_amount' : '';
                    return '<span class="display_currency ' . $class . '" data-currency_symbol = true data-orig-value="' . $row->free_issues_amount . '">' . $row->free_issues_amount . '</span>';
                })

                ->rawColumns(['current_stock','free_issues_amount', 'total_free_issue_quantity', 'subtotal', 'total_qty_sold', 'category_name'])
                ->make(true);
        }
    }
