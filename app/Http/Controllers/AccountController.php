...
class AccountController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                            ->addColumn(
                                ...
                                @if(auth()->user()->can("access_payment_account_deposite"))<button data-href="{{action(\'AccountController@getWithdraw\',[$id])}}" class="btn btn-xs btn-primary btn-modal" data-container=".view_modal"><i class="fas fa-money-bill-alt"></i> Withdraw</button>@endif
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (!empty($row->sub_type)) {
            ...
            if (in_array($row->sub_type, ['fund_transfer', 'deposit', 'withdraw']) && !empty($row->transfer_transaction)) {
                if ($row->type == 'credit') {
