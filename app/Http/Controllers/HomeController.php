...
class HomeController extends Controller
{
    ...
    public function smsBalance()
    {
        ...
        $sms_service_provider = $business->sms_service_provider;

        if ($sms_service_provider == 'default') {
        ...
        } else if ($sms_service_provider == 'dialog') {

            if(empty(config('app.dialog_username')) || empty(config('app.dialog_password')) || empty(config('app.dialog_sms_login'))){
                return null;
            }
            $body = (object) [
    ...
    public function setLoginImage()
    {
        $business = Business::first();
        return $business->login_image_url;
    }

    public function getApiDetails()
    {
        $client = new Client();
        $response = $client->get('https://api.clickypos.com/api/api-check');
        $contents = (string) $response->getBody()->getContents();
        dd($contents);
    }

    public function postApiDetails(Request $request)
    {
        $array = [1, 2, 3];
        $client = new Client();
        $res = $client->request('POST', 'https://api.clickypos.com/api/post-data', [
            'form_params' => [
                'client_id' => 'test_id',
                'secret' => 'test_secret',
                'array' => $array,
    ...
    public function Dialog()
    {
        ...
        $send_url = config('app.dialog_send_sms_url');
        $ch = curl_init($url);
