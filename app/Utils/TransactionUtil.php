...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'type' => $sale_type,
            'status' => $input['status'],
            'free_issues_amount' => $input['free_issues_amount'],
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        ...
                                        'free_issue_quantity' => !empty($product['free_quantity']) ? $product['free_quantity'] : '0',
    ...
    private function __makeLinesForComboProduct($combo_items, $parent_sell_line)
    {
        ...
        foreach ($combo_items as $value) {
            ...
            $combo_lines[] = new TransactionSellLine([
                ...
                'free_issue_quantity' => !empty($value['free_quantity']) ? $value['free_quantity'] : '0',
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if (!empty($line->free_issue_quantity)) {
                $free_issue_quantity = $line->free_issue_quantity;
            } else {
            ...
            $line_array = [
                ...
                'free_issue_quantity' => $this->num_f($free_issue_quantity, false, $business_details, true),
    ...
    public function getTransactionTotals(
        ...
        if (in_array('sell', $transaction_types)) {
            $query->addSelect(
                ...
                DB::raw("SUM(IF(transactions.type='sell' AND transactions.status='final', free_issues_amount, 0)) as free_issues_amount")
            );
        }
        ...
        if (in_array('sell', $transaction_types)) {
            ...
            $output['free_issues_amount'] =
                !empty($transaction_totals->free_issues_amount) ?
                $transaction_totals->free_issues_amount : 0;
        }
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        //free issues
        $data['free_issues_amount'] = $transaction_totals['free_issues_amount'];
        ...
        $data['net_profit'] = $module_total + $gross_profit
            ...
            ) - ($data['total_reward_amount'] + $data['total_expense'] + $data['total_adjustment'] + $data['total_transfer_shipping_charges'] + $data['total_purchase_shipping_charge'] + $data['total_purchase_additional_expense'] + $data['total_sell_discount'] + $data['free_issues_amount']
