...
class Util
{
    ...
    public function sendSms($data)
    {
        ...
        $actual_number = (substr($number, -9));

        $request_data = [
            $sms_settings['send_to_param_name'] => '+94' . $actual_number,
    ...
    public function sendDialogSms($data)
    {
        $number = $data['mobile_number'];
        $send_no = (substr($number, -9));

        $mobile_array = [
            ['mobile' => $send_no],
        ];
        $esms_token = DialogEsmsToken::latest()->first();
        $token = null;
        if (empty($esms_token)) {
            $output = $this->dialogSmsLogin();
            $DialogEsmsToken = new DialogEsmsToken;
            $DialogEsmsToken->esms_token = $output;
            $DialogEsmsToken->token_created_date = now();
            $DialogEsmsToken->save();
            $token = $output;
        } else {
            $date = Carbon::now()->subHours(11);
            $token_created_date = $esms_token->token_created_date;
            if ($token_created_date < $date) {
                $output = $this->dialogSmsLogin();
                $esms_token->esms_token = $output;
                $esms_token->token_created_date = now();
                $esms_token->update();
                $token = $output;
            } else {
                $token = $esms_token->esms_token;
            }
        }
        ...
        $details = [
            ...
            'sourceAddress' => config('app.dialog_mask'),
            'transaction_id' => $current_date,
        ];
        $body = json_encode($details);
        $url = config('app.dialog_send_sms_url');
        $ch = curl_init($url);
    ...
    public function dialogSmsLogin()
    {
        $url = config('app.dialog_sms_login');
        $body = (object) [
            'username' => config('app.dialog_username'),
            'password' => config('app.dialog_password'),
        ];
