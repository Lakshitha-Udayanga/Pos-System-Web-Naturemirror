...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax,$product_qty, $free_qty,  $combo_variations = [])
    {
        ...
        $ProductHasFreeStock = ProductHasFreeStock::create([
            ...
            'free_qty' => $this->num_uf($free_qty),
            'product_qty' => $this->num_uf($product_qty),
    ...
    {
        $business_details = Business::find($transaction->business_id); //automatic create product variation depend on purchase price
        ...
        foreach ($input_data as $data) {
            ...
            if ($business_details->product_variation_on_purchase == 1) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                if ($business_details->product_variation_on_purchase == 1 && !$new_variation) {
            ...
            //free issue
            $purchase_line->free_issues = isset($data['free_issue_quantity']) ? $data['free_issue_quantity'] : 0;
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        ...
        $sell_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->select(
                ...
                DB::raw("SUM(IF(t.type='sell', sl.free_issue_quantity, 0)) as free_stock"),
            )
            ->get()->first();
        ...
        $output = [
            ...
            'free_stock' => $sell_details->free_stock ?? 0
        ];
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        ...
        foreach ($stock_history as $stock_line) {
            ...
            if ($stock_line->transaction_type == 'sell') {
                ...
                - $this->roundQuantity($stock_line['free_issue_quantity']);
                $new_quantity_change =  -1 * $stock_line->sell_line_quantity;
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $new_quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'free_issue_quantity' => $this->roundQuantity($stock_line['free_issue_quantity']),
                    'type' => 'sell',
