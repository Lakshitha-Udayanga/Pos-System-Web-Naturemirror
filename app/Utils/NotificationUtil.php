        ...
        if (!empty($notification_template)) {
            if (!empty($notification_template->auto_send) || !empty($notification_template->auto_send_sms) || !empty($notification_template->auto_send_wa_notif)) {
                ...
                if (!empty($notification_template->auto_send_sms)) {
                    ...
                    //send sms only one number
                    $send_sms_only_one_number = $business->send_sms_only_one_number;

                    if ($send_sms_only_one_number != 0) {
                        $data['mobile_number'] = $business->default_sms_number;
                    }

                    if (!empty($data['mobile_number'])) {

                        try {
                            if ($business->sms_service_provider == 'default') {
                                $this->sendSms($data);
                            } else if ($business->sms_service_provider == 'dialog') {
                                $this->sendDialogSms($data);
                            }
