$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('keyup', 'input.pos_quantity', function() {
        ...
        adjustComboQty(tr);
        var free_issues_enable = __read_number(tr.find('input.input_free_issues_enable')); 
if(free_issues_enable == true){
        var free_qty = __read_number(tr.find('input.input_free_qty'));
        var product_qty = __read_number(tr.find('input.input_product_qty'));
        free_isses_qty = entered_qty/product_qty*free_qty;
            var result = Math.floor(free_isses_qty);
        __write_number(tr.find('input.free_quantity'), result, false, 2);

}
        var purchase_price = __read_number(tr.find('input.input_purchase_price'));
        var free_line_total = result * purchase_price;
        __write_number(tr.find('input.input_free_issues_amount'), free_line_total, false, 2);
   });
    $('table#pos_table tbody').on('change', 'input.free_quantity', function() {
        var free_quantity = __read_number($(this));
        // var purchase_price = __read_number(tr.find('input.input_purchase_price'));
        var purchase_price = $('#purchase_price').val();
        var free_line_total = free_quantity * purchase_price;
        $("#free_issues_amount").val(free_line_total);
        // __write_number(tr.find('input.input_free_issues_amount'), free_line_total, false, 2);
