...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/purchases/get_purchase_entry_row_pop',
            dataType: 'html',
            data: data,
            success: function(result) {
...
$('button#submit_purchase_form, button#save-and-print').click(function() {
    ...
    var data = $('#add_purchase_form').serialize();
        ...
        success: function(result) {
            if (result.success == 1) {
                ...
                // if (result.receipt.is_enabled) {
                    $('table#purchase_entry_table tbody tr').remove();
                    $('#row_count').val('0');
                    $('#total_quantity').text('0.00');
                    $('#total_subtotal').text('0.00');
                    $('#total_subtotal_input').val('0');
                    $('#grand_total_hidden').val('0');
                    $('#grand_total').text('0');
                    $('#discount_calculated_amount').text('0.00');
                    $('input#search_product').select();
...
function set_default_supplier() {
    ...
    var exists = default_supplier_id ? $('select#supplier_id option[value=' + default_supplier_id + ']').length : 0;
    if (exists == 0 && default_supplier_id) {
        $('select#supplier_id').append(
