...
<tr class="product_row" data-row_index="<?php echo e($row_count, false); ?>"
    <?php if(!empty($so_line)): ?> data-so_id="<?php echo e($so_line->transaction_id, false); ?>" <?php endif; ?>>
    <td>
        ...
        <?php if(($edit_price || $edit_discount) && empty($is_direct_sell)): ?>
            <div title="<?php echo app('translator')->getFromJson('lang_v1.pos_edit_product_price_help'); ?>">
                <span class="text-link text-info cursor-pointer" data-toggle="modal"
                    data-target="#row_edit_product_price_modal_<?php echo e($row_count, false); ?>">
        ...
        <?php if(empty($is_direct_sell)): ?>
            <div class="modal fade row_edit_product_price_model" id="row_edit_product_price_modal_<?php echo e($row_count, false); ?>"
        ...
        <?php if(in_array('modifiers', $enabled_modules)): ?>
            <div class="modifiers_html">
                <?php if(!empty($product->product_ms)): ?>
                    <?php echo $__env->make('restaurant.product_modifier_set.modifier_for_product', [
                        'edit_modifiers' => true,
            ...
            <?php if(!empty($product->lot_numbers) && empty($is_sales_order)): ?>
                <select class="form-control lot_number input-sm" name="products[<?php echo e($row_count, false); ?>][lot_no_line_id]"
                    ...
                    <?php $__currentLoopData = $product->lot_numbers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lot_number): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        ?>
                        <option value="<?php echo e($lot_number->purchase_line_id, false); ?>"
                            data-qty_available="<?php echo e($lot_number->qty_available, false); ?>" data-msg-max="<?php echo app('translator')->getFromJson('lang_v1.quantity_error_msg_in_lot', ['qty' => $lot_number->qty_formated, 'unit' => $product->unit]); ?>"
                            <?php echo e($selected, false); ?>>
                            <?php if(!empty($lot_number->lot_number) && $lot_enabled == 1): ?>
                                <?php echo e($lot_number->lot_number, false); ?>

                                <?php endif; ?> <?php if($lot_enabled == 1 && $exp_enabled == 1): ?>
                                    -
                                    <?php endif; ?> <?php if($exp_enabled == 1 && !empty($lot_number->exp_date)): ?>
                                        <?php echo app('translator')->getFromJson('product.exp_date'); ?>: <?php echo e(\Carbon::createFromTimestamp(strtotime($lot_number->exp_date))->format(session('business.date_format')), false); ?>

                                    <?php endif; ?> <?php echo e($expiry_text, false); ?>
    ...
    <td>
        ...
        <div class="input-group input-number">
            ...
            <input type="text" data-min="1" class="form-control pos_quantity input_number mousetrap input_quantity"
                value="<?php echo e(number_format($product->quantity_ordered, session('business.quantity_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>"
			...
			<?php else: ?> 
				...
				data-msg-abs_digit="<?php echo app('translator')->getFromJson('lang_v1.decimal_value_not_allowed'); ?>" <?php endif; ?>
                data-rule-required="true" data-msg-required="<?php echo app('translator')->getFromJson('validation.custom-messages.this_field_is_required'); ?>"
                <?php if($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order)): ?> data-rule-max-value="<?php echo e($max_qty_rule, false); ?>" data-qty_available="<?php echo e($product->qty_available, false); ?>" data-msg-max-value="<?php echo e($max_qty_msg, false); ?>" 
        ...
        <?php if(!empty($product->second_unit)): ?>
            ...
            <input type="text" name="products[<?php echo e($row_count, false); ?>][secondary_unit_quantity]"
        ...
        <input type="hidden" class="hidden_base_unit_sell_price"
            value="<?php echo e($product->default_sell_price / $multiplier, false); ?>">
    ...
    <?php
        $f_product = App\Product::select('id', 'free_issues_enable')
            ->where('id', $product->product_id)
            ->first();
        $purchase_price = App\PurchaseLine::select('purchase_price_inc_tax', 'product_id', 'variation_id')
            ->where('product_id', $product->product_id)
            ->where('variation_id', $product->variation_id)
            ->first();
        $product_has_free_stocks = App\ProductHasFreeStock::where('product_id', $product->product_id)->first();
        if ($f_product->free_issues_enable == false) {
            $free_issues_enable = 0;
        } else {
            $free_issues_enable = 1;
        }
        if ($product_has_free_stocks) {
            $product_qty = $product_has_free_stocks->product_qty;
            $free_qty = $product_has_free_stocks->free_qty;
        } else {
            $product_qty = 0;
            $free_qty = 0;
        }
    ...
    <td>
        <div class="input-group input-number">
            <span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-down"
                    <?php if($f_product->free_issues_enable == false): ?> disabled <?php endif; ?>><i
                        class="fa fa-minus text-danger"></i></button></span>
            <input type="text" data-min="1" class="form-control  input_number mousetrap free_quantity"
                value="<?php echo e(number_format($product->free_issue_quantity, session('business.quantity_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>"
                name="products[<?php echo e($row_count, false); ?>][free_quantity]"
                <?php if($f_product->free_issues_enable == false): ?> readonly <?php endif; ?>>
            <span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up"
                    <?php if($f_product->free_issues_enable == false): ?> disabled <?php endif; ?>><i
                        class="fa fa-plus text-success"></i></button></span>
        </div>
        <input type="hidden" class="form-control input_number  input_free_issues_enable" id="free_issues_enable"
            name="free_issues_enable" value="<?php echo e($free_issues_enable, false); ?>">
        <input type="hidden" class="form-control input_number  input_product_qty" id="product_qty"
            name="product_qty" value="<?php echo e($product_qty, false); ?>">
        <input type="hidden" class="form-control input_number input_free_qty" id="free_qty" name="free_qty"
            value="<?php echo e($free_qty, false); ?>">
        <input type="hidden" class="form-control free_issues_amount input_free_issues_amount"
            id="free_issues_amount" name="free_issues_amount" value="">
        <input type="hidden" class="form-control purchase_price input_purchase_price" id="purchase_price"
            name="purchase_price" value="<?php echo e($purchase_price->purchase_price_inc_tax, false); ?>">
    </td>
    <?php if(!empty($is_direct_sell)): ?>
        ...
        <td class="<?php if(!auth()->user()->can('edit_product_price_from_sale_screen')): ?> hide <?php endif; ?>">
            <input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price]"
                class="form-control pos_unit_price input_number mousetrap" value="<?php echo e(number_format($pos_unit_price, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>"
        ...
        <td <?php if(!$edit_discount): ?> class="hide" <?php endif; ?>>
            <?php echo Form::text("products[$row_count][line_discount_amount]", number_format($discount_amount, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), [
            ...
            <?php if(!empty($discount)): ?>
                <p class="help-block"><?php echo __('lang_v1.applied_discount_text', [
    ...
    <td class="<?php echo e($hide_tax, false); ?>">
        <input type="text" name="products[<?php echo e($row_count, false); ?>][unit_price_inc_tax]"
            class="form-control pos_unit_price_inc_tax input_number" value="<?php echo e(number_format($unit_price_inc_tax, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>"
    ...
    <?php if(!empty($common_settings['enable_product_warranty']) && !empty($is_direct_sell)): ?>
        <td>
            <?php echo Form::select("products[$row_count][warranty_id]", $warranties, $warranty_id, [
    ...
    <td class="text-center">
        ...
        <input type="<?php echo e($subtotal_type, false); ?>"
            ...
            value="<?php echo e(number_format($product->quantity_ordered * $unit_price_inc_tax, session('business.currency_precision', 2), session('currency')['decimal_separator'], session('currency')['thousand_separator']), false); ?>">
...
<?php /**PATH F:\My Project\Thand\free-isses-item\New folder\resources\views/sale_pos/product_row.blade.php ENDPATH**/ ?>