...
<header class="main-header no-print">
    <a href="<?php echo e(route('home'), false); ?>" class="logo">
        ...
        <span class="logo-lg"><?php echo e(Session::get('business.name'), false); ?> <i class="fa fa-circle text-success"
    ...
    <nav class="navbar navbar-static-top" role="navigation">
        ...
        <?php if(!empty(session('previous_user_id')) && !empty(session('previous_username'))): ?>
            <a href="<?php echo e(route('sign-in-as-user', session('previous_user_id')), false); ?>"
                class="btn btn-flat btn-danger m-8 btn-sm mt-10"><i class="fas fa-undo"></i> <?php echo app('translator')->getFromJson('lang_v1.back_to_username', ['username' => session('previous_username')]); ?></a>
        <?php endif; ?>
        ...
        <div class="navbar-custom-menu">
            ...
            <?php if($business->sms_dashbord_url != ''): ?>
                
                <a href="<?php echo e($business->sms_dashbord_url, false); ?>" target="_blank" title="<?php echo app('translator')->getFromJson('SMS'); ?>"
                    data-toggle="tooltip" data-placement="bottom"
                    class="btn btn-flat pull-left m-8 btn-sm mt-10 btn-warning text-black">
                    <strong><i class="fa fa-comments fa-lg" aria-hidden="true"> <?php echo app('translator')->getFromJson('SMS'); ?></i></strong>
                </a>
            <?php else: ?>
                
                <button class="btn btn-flat pull-left m-8 btn-sm mt-10 btn-warning text-black"
                    id="btn_no_sms"><strong><i class="fa fa-comments fa-lg" aria-hidden="true">
                            <?php echo app('translator')->getFromJson('SMS'); ?></i></strong></button>
            <?php endif; ?>
            ...
            <?php if(in_array('pos_sale', $enabled_modules)): ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sell.create')): ?>
                    <a href="<?php echo e(action('SellPosController@create'), false); ?>" title="<?php echo app('translator')->getFromJson('sale.pos_sale'); ?>" data-toggle="tooltip"
            ...
            <?php
                $super_admin = App\SuperAdmin::first();
            ?>
            ...
            <?php if($super_admin->enable_short_cut_purchase ==  1): ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('purchase.create')): ?>
                    <a href="<?php echo e(url('/purchases/pop'), false); ?>" target="_blank" title="POP" data-toggle="tooltip"
                        data-placement="bottom" class="btn btn-flat pull-left m-8 btn-sm mt-10 btn-success">
                        <strong><i class="fa fa-cubes" aria-hidden="true"></i> &nbsp; POP</strong>
                    </a>
                <?php endif; ?>
            <?php endif; ?>

            <?php if(Module::has('Repair') && $business->repair == '1'): ?>
...
<div class="modal fade" id="sms_banner_modal" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <!--Modal body with image-->
            <div class="modal-body">
                <img class="img-responsive" src="<?php echo e(asset('img/sms_banner.jpeg'), false); ?>" />
            </div>
        </div>
    </div>
</div>
<?php /**PATH F:\My Project\Thand\free-isses-item\New folder\resources\views/layouts/partials/header.blade.php ENDPATH**/ ?>