<div class="row">
	...
	<div class="form-group">
		<label>
		{!! Form::checkbox('enable_cutomer_due', 1, false, ['class' => 'input-icheck', 'id' => 'enable_cutomer_due']); !!} <strong>@lang('stock_adjustment.cutomer_due')</strong>
            </label>
		<h5 class="contact_due_text"><strong>@lang('account.customer_due'):</strong>
			<span></span></h5>
			{!! Form::hidden("payment[$row_index][due_payment_type]", 'sell'); !!}
			{!! Form::hidden("payment[$row_index][due_amount]", '', ['class' => 'due_amount']); !!}
	</div>
	<hr>
	<div class="{{$col_class}}">
		<div class="form-group">
			{!! Form::hidden("payment[$row_index][replace_amount]", @num_format($payment_line['amount']), ['class' => 'form-control replace-amount input_number', 'required', 'id' => "amount_$row_index", 'placeholder' => __('sale.amount'), 'readonly' => $readonly]); !!}
