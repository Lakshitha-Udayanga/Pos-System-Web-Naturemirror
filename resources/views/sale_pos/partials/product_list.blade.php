@forelse($products as $product)
    <div class="col-md-3 col-xs-4 product_list no-print">
        <div class="product_box" data-variation_id="{{ $product->id }}"
            ...
            <small style="color: red">@if(!empty($product->qty_available)){{ number_format($product->qty_available) }} @if (!empty($product->short_name)) {{ $product->short_name }} @else {{ $product->actual_name }} @endif @else -- @endif</small>
