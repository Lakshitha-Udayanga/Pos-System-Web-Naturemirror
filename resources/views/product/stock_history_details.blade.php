<div class="row">
	...
	<div class="col-md-4 col-xs-4">
		...
		<table class="table table-condensed">
			...
			<tr>
				<th style="color: red;">@lang('Total Free Stock')</th>
				<td>
					<span class="display_currency" data-is_quantity="true" style="color: red;">{{$stock_details['free_stock']}}</span> <span style="color: red;">{{$stock_details['unit']}}</span>
				</td>
			</tr>
		</table>
	</div>
		...
		<table class="table table-slim" id="stock_history_table">
			...
			@forelse($stock_history as $history)
				<tr>
					...
					@endif
					<td>
						<span class="display_currency text-danger" data-is_quantity="true">{{$history['free_issue_quantity']}}</span> 
					</td>
