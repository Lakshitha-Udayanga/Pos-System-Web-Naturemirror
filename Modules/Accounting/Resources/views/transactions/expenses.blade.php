            ...
            contact_table = $('#contact_table').DataTable({
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        ...
                        if ($('#has_purchase_due').length > 0 && $('#has_purchase_due').is(
                            ':checked')) {
                            d.has_purchase_due = true;
                        }

                        if ($('#has_purchase_return').length > 0 && $('#has_purchase_return').is(
                                ':checked')) {
                        ...
                        if ($('#has_opening_balance').length > 0 && $('#has_opening_balance').is(
                                ':checked')) {
                ...
                $('div.customer_groups_modal').load($(this).data('href'), function() {
                    ...
                    $('form#customer_group_edit_form').submit(function(e) {
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                if (result.success == true) {
                                    $('div.customer_groups_modal').modal(
                                    'hide');
                                    toastr.success(result.msg);
                                    customer_groups_table.ajax.reload();
                                } else {
                    ...
                    if (result) {
                        ...
                        $('#product_form_part').find('.variation_value_name').each(function() {
                            ...
                            if ($(this).closest('tr').find('.row_variation_id')) {
                                row_variation_id = $(this).closest('tr').find('.row_variation_id')
                            .val();
                            }
                            element.rules("add", {
                                remote: {
        ...
        function updateStockBySellingPrice(data) {
            if ($('#closing_stock_by_sp').length > 0) {
                $.ajax({
                    ...
                    data: data,
                    success: function(result) {
                        $('#closing_stock_by_sp').html(__currency_trans_from_en(result.closing_stock_by_sp),
                            true);
                        $('#opening_stock_by_sp').html(__currency_trans_from_en(result.opening_stock_by_sp),
                            true);
                    },
                });
            }
